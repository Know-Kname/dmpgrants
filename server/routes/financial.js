...
router.post('/deposits', validateDeposit, asyncHandler(async (req, res) => {
  ...
  const result = await query(
    ...
    [amount, date, method, reference || null, customerIdValue, notes || null, req.user.id]
  );
...
router.put('/receivables/:id', validateUUIDParam, validateReceivableUpdate, asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `UPDATE accounts_receivable SET amount_paid = COALESCE($1, amount_paid), status = COALESCE($2, status), updated_at = CURRENT_TIMESTAMP
     WHERE id = $3 RETURNING *`,
    [amountPaidValue, status, id]
  );
...
router.put('/payables/:id', validateUUIDParam, validatePayableUpdate, asyncHandler(async (req, res) => {
  ...
  const result = await query(
    `UPDATE accounts_payable SET amount_paid = COALESCE($1, amount_paid), status = COALESCE($2, status), updated_at = CURRENT_TIMESTAMP
     WHERE id = $3 RETURNING *`,
    [amountPaidValue, status, id]
  );
  res.json(result.rows[0]);
